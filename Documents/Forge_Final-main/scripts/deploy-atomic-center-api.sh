...
LOCAL_WEBSERVER="$ROOT/atomic/MeshCentral-master/webserver.js"
# Customized login/message templates that back the forgot-password (choose-your-own-password) flow.
LOCAL_VIEW_FILES=(
...
if [[ -f "$LOCAL_WEBSERVER" ]]; then
  echo "Uploading webserver.js (forgot-password flow)…"
  "${SCP[@]}" "$LOCAL_WEBSERVER" "$AWS_HOST:/tmp/webserver.js"
  "${SSH[@]}" "$AWS_HOST" "sudo cp /tmp/webserver.js ~/$AWS_MESH_DIR/webserver.js"
fi

echo "Uploading login/message templates (forgot-password UI)…"
for vf in "${LOCAL_VIEW_FILES[@]}"; do
  if [[ -f "$ROOT/atomic/MeshCentral-master/$vf" ]]; then
    base="$(basename "$vf")"
    "${SCP[@]}" "$ROOT/atomic/MeshCentral-master/$vf" "$AWS_HOST:/tmp/$base"
    "${SSH[@]}" "$AWS_HOST" "sudo cp /tmp/$base ~/$AWS_MESH_DIR/views/$base"
  fi
done
