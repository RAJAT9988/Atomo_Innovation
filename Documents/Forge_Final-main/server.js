...
const demoAuth = require('./lib/demo-auth');
...
app.get('/login', (_req, res) => {
  const snap = meshcentralStatus.getSnapshot();
  if (snap.stale && !demoAuth.isDemoLoginEnabled()) meshcentralStatus.refreshInBackground();
  const boot = {
    ...deviceStatusPayload(),
    online: demoAuth.isDemoLoginEnabled() ? true : snap.reachable,
    ...demoAuth.getDemoPublicInfo(),
  };
...
app.get('/api/device/status', (_req, res) => {
  if (meshcentralStatus.isStale() && !demoAuth.isDemoLoginEnabled()) {
    meshcentralStatus.refresh().catch(() => {});
  }
  const demoInfo = demoAuth.getDemoPublicInfo();
  res.json({
    ...deviceStatusPayload(),
    meshcentralReachable: meshcentralStatus.getReachable(),
    online: demoInfo.demoLogin ? true : meshcentralStatus.getReachable(),
    ...demoInfo,
  });
});
